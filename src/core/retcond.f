...
c
c inputs :
c     chg   :double precision     :cost of a entry - sold position in unit of returns
c     split :double precision     :splithedge in % of the return
c     i     :integer  :time bar to be calculated
c     xrets :read(1:i) :input asset return time series of the prices 
c     xinds :double precision(1:i) :input indicator time series
c
c outputs :
c     ys    :double precision(1:i) : output time series where the condret must be stored

      implicit none
      integer i, ix
      double precision chg, slip, cost 
      double precision ys(1:i), xrets(1:i), xinds(1:i)
      integer i1
      i1 = i-1
      if (i .eq. 1) then
        ys(i) = 0.
